...
resource "aws_subnet" "elastic_subnet"{
  for_each = {us-east-1a="172.20.10.0/24",us-east-1b="172.20.20.0/24",us-east-1c="172.20.30.0/24"}
  vpc_id = aws_vpc.elastic_vpc.id
  availability_zone = each.key
  cidr_block = each.value
  tags={
    Name="${var.name_tag}_subnet_${each.key}"
  }
}

locals {
  az_name = ["${var.aws_region}a","${var.aws_region}b","${var.aws_region}c"]
}
...
resource "aws_security_group" "logstash_sg" {
  ...
  tags={
    Name="${var.name_tag}_logstash_sg"
  }
}
...
resource "aws_security_group" "filebeat_sg" {
  ...
  tags={
    Name="${var.name_tag}_filebeat_sg"
  }
}
